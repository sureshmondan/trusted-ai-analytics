...
|---|---|---|
| **Approved Numbers** | Is this result consistent with what experienced people on this team know the numbers should look like? | Every senior analyst, data lead, and ops manager carries a mental model of the business — thresholds, expected ranges, known patterns. That tribal knowledge isn't in the schema. It isn't in the semantic layer. The LLM starts from zero every query. The Approved Numbers Store encodes what your team already knows, so the AI inherits it instead of flying blind. |
...
- **Approved Numbers Store**: A systematic encoding of tribal knowledge — the mental model experienced analysts, data leads, and finance teams carry about what the numbers should look like. That knowledge isn't in the schema. It isn't in the semantic layer. When a senior person leaves, it walks out with them. The LLM starts from zero on every query. The store captures historical human-approved figures as a sanity baseline: not to freeze the DW or replace it as a reporting source, but to give the AI the one thing it currently lacks — institutional context. When the system returns a number that's wildly outside the expected range, the Validation Agent catches it before it reaches a deck. It backfills from sources already in your stack — dbt snapshots, period-end summary tables, BI-certified datasets — no upstream schema changes, no migration.
