...
|---|---|---|
| **Approved Numbers** | Is this result in the ballpark of what Finance has historically approved for this metric? | Finance blessed ~$4.2B at Q3 close. That expectation lives in a signed-off report, not in the AI's context. The LLM returns $8.4B with a clean confidence score and nothing flags it — because nothing in the stack knows what the business expects the number to look like. |
...
- **Approved Numbers Store**: A history of human-approved figures used as a sanity baseline — not a replacement reporting source. The DW remains the system of record. What the store captures is the business expectation: Finance closed Q3 and blessed ~$4.2B revenue. That expectation lives in a signed-off report, in the CFO's email. The LLM has zero access to any of it. So when it returns $8.4B with a clean confidence score, nothing flags it — because nothing in the stack knows what Finance expects the number to look like. The Validation Agent does: it checks the result against the store, and if it's outside the approved ballpark, it surfaces the deviation before it reaches a slide deck. The store is not about freezing numbers — it's about giving the AI the one thing it currently lacks: human business context. It backfills from sources already in your stack — dbt snapshots, period-end summary tables, BI-certified datasets — no upstream schema changes, no migration.
