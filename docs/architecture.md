...
**The core problem this solves**: The AI has no access to tribal knowledge. Every experienced data person carries a mental model of what the numbers should look like — a senior analyst knows Q4 revenue always looks inflated, a data lead knows churn and revenue shouldn't move in opposite directions, an ops manager knows the threshold that would trigger a board call. None of that lives in a schema. None of it is in the semantic layer. When that person leaves, it walks out with them. The LLM starts from zero on every query. The Approved Numbers Store is how you systematically encode that institutional knowledge so the AI inherits it — turning what lives in people's heads into a validation layer the system can actually use.
